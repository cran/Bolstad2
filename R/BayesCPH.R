...
#' 
#' Uses a Metropolis Hastings scheme on the proportional hazards model to draw
...
#' 
#' 
#' @param y the Poisson censored response vector. It has value 0 when the
...
#' @export BayesCPH
BayesCPH = function(y, t, x, steps = 1000, priorMean = NULL, priorVar = NULL, mleMean = NULL, mleVar,
    ...
    for (j in 1:nObs) BetaXt[, j] = -t[j] * BetaXt[, j] + y[j] * log(t[j] * BetaXt[, j])
