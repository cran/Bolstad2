normGibbs<-function(y, steps = 1000, type = 'ind', ...){
    ...
    if(type == 'ind'){
        ...
        temp.df<-data.frame(SSt = rep(0, steps), s1 = rep(0, steps),
                            kappa1 = rep(0, steps), prec = rep(0, steps),
                            v = rep(0, steps), std = rep(0, steps),
                            mean = rep(0, steps))

        muSample <- rep(0, steps)
        muSample[1] <- z[1]*sqrt(v1)+m1

        temp.df[1,]<-c(SSt, s1, kappa1, prec1, v1, sqrt(v1), m1)
        ...
        varSample <- rep(0, steps)
        temp.df<-data.frame(SSt = rep(0, steps), s1 = rep(0, steps), kappa1 = rep(0, steps),
                            prec1 = rep(0, steps), v1 = rep(0, steps), m1 = rep(0, steps))

        varSample[1] <- s1/Chi[1]
        temp.df$SSt[1] <- SSt
        temp.df$s1[1] <- s1
        temp.df$kapp1[1] <- kappa1
